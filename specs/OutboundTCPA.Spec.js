describe ('TCPA Safe Dialling Tests',function(){
    ...
    it('Safe Dialling Login test',function(){
        AgentLoginPage.launch();
        AgentLoginPage.EnterUserName(testdata[0].TCPAAgentUserName);
        AgentLoginPage.EnterPassword(testdata[0].TCPAAgentPassword);
        AgentLoginPage.SignIn();
        browser.wait(EC.visibilityOf(AgentConfigPage.DialGroupList,10000));
        AgentConfigPage.GetDialGroupList(); 
        AgentConfigPage.SelectDialGroup(testdata[0].TCPADialGroup);    
    ...
    it('Admin Login test',function(){
        ...
        browser.sleep(20000);
        expect(AdminHomePage.AccountName()).toEqual('TCPA Test Account'); 
    });
    

// Real-Time Outbound telemetry tests :
    it('TCPA Outbound Telemetry Test',async function(){
        ...
        for(var i=0;i<Outtelemetry.NameColumn.length;i++){
            if(Outtelemetry.NameColumn[i]=== testdata[0].TCPACampaign){
    ...
    it('TCPA Disposition Test',function(){ 
        browser.getAllWindowHandles().then(function (handles) {
            browser.switchTo().window(handles[0]);
        })
    ...
    it('Second OutBoundTCPA Call test',function() {
        AgentOutboundDialerPage.DialLead();
        browser.wait(EC.visibilityOf(AgentDialPadPage.RequiredCheck,4000));
