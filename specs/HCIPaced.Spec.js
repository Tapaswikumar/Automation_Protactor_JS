describe('HCI Paced Tests', function() {
    ...
    it('Non-Clicker agent login test',function(){
        ...
        AgentConfigPage.SelectDialGroup(testdata[0].HCIPacedGroup);    
    ...
    it('Lead Fetch test',function(){
        ...
        browser.wait(EC.visibilityOf(AgentOutboundDialerPage.FetchedLead,10000));
    ...
    it('Outbound Telemetry Test',async function(){
        ...
        for(var i=0;i<Outtelemetry.NameColumn.length;i++){
            if(Outtelemetry.NameColumn[i]=== testdata[0].HCIPacedCampaign){
    ...
    it('Agent Telemetry Test',async function(){
        ...
        for(var i=0;i<Agenttelemetry.NameColumn.length;i++){
            if(Agenttelemetry.NameColumn[i]=== testdata[0].HCIAgentName){
                this.Clickeragentindex=i;
            }
            if(Agenttelemetry.NameColumn[i]=== testdata[0].AgentName){
                this.NonClickeragentindex=i;
            }
        }
        expect(Agenttelemetry.Col_2[this.Clickeragentindex]).toEqual("Working");
        expect(Agenttelemetry.Col_2[this.NonClickeragentindex]).toEqual("ENGAGED");
    });
    ...
    it('HCI Paced Dialing test',function(){
        browser.getAllWindowHandles().then(function (handles) {
